...
function addCodemirror(card_id, textarea, fullscreen) {
    ...
    let original_textarea = document.querySelector(card_id).querySelector('.mousetrap');
    ...
    if (textarea) {
        let codeMirrorWrapper = CodeMirror.fromTextArea(
          textarea,
          {
            mode: "markdown",
            lineWrapping: true,
        ...
        codeMirrorWrapper.on("change", function(cm, change) {
            ...
            cm.save();
            document.querySelector(card_id).querySelector('.mousetrap').value = cm.getValue();
...
function run() {
  ...
  CodeMirror.Vim.defineEx('q', null, function(cm) {cm.display.input.blur(); /* cm.toTextArea(); */ Backbone.trigger("key:cancel");});
  CodeMirror.Vim.defineEx('wq', null, function(cm) {cm.save(); cm.display.input.blur(); /* cm.toTextArea(); */Backbone.trigger("key:save");});
